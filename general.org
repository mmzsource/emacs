...
- Monitors =C-x C-f= (a.o.) to detect when you try to access remote files
- All tramp connections follow this syntax: =/protocol:[user@]hostname[#port]:
  (e.g. =/ssh:homer@powerplant:/var/log/reactor.log=)
- Supports many protocols, you'll most likely use =ssh=
- Parses your =~/.ssh/config= file and auto-completes known hosts when you have
  entered =ssh= as your protocol.
- Invoking commands like =C-x d= (Dired - copying files from remote to local
  also works transparently), =M-x rgrep=, =M-x shell=, =M-x eshell=, etc. All
  commands are tunnelled through the remote session and the results are
  displayed in Emacs.
- You can request sudo access when on a remote server. To request sudo access
  to =/etc/stab=: =/sudo:root@localhost:/etc/fstab=
- It's also possible to multi-hop with TRAMP:
  =/ssh:homer@powerplant|sudo:powerplant:/root/salary.txt= First, we connect
  with =powerplant= as =homer=. Then another 'connection' invokes =sudo= and
  opens =/root/salary.txt= as a sudo user.
- You can also bookmark remote files with =C-x r b= and TRAMP will automatically
  reconnect if you open a bookmark later with =C-x r b= or =C-x r l=.

** EWW - Emacs Web Wowser

| Keybinding       | Description                                  |
|------------------+----------------------------------------------|
| =M-x eww=        | Open eww and insert an URL or search term    |
| =M-x browse-url= | To open things in a web browser              |
| =C-x / <key>=    | Activate (search)engine mode. See config.org |
|------------------+----------------------------------------------|
...
So if you select the previous character combinations and type =M-|= sort, the
output will be sorted in the mini-buffer. =C-u M-|= will /replace/ the region
with the output of the command.

Furthermore there's also something like =M-x compile= that can do a lot more
than compilation.
...
=M-x compile= =java -version=

That will create a buffer with the results of running that command in the shell.

=M-x compile= assumes you want to use =make=, so simply delete that if you want
to fire off another shell command.

** Shells

There are three ways of interacting with shells in Emacs: =M-x shell= (a wrapper
around an existing shell), =M-x eshell= (a complete shell implementation written
in elisp) and =M-x ansi-term= (a terminal emulator).

(for comparison, run the three of them with the =top= command.)

*** =M-x shell= - shell mode

- calls underlying bash implementation and redirects =stdin=, =stdout= and
  =stderr=
- Because of the IO redirection, =TAB= completion for bash commands won't work,
  =TAB= completion for hostnames, files and paths, etc. do work however.
- Because it's completely text, you can kill and yank, but things like =top= and
  =man= don't work quite well (but in those particular cases you can also use
  =M-x proced= and =M-x man= respectively)

*** =M-x eshell= - elisp shell

- Complete shell written in elisp and communicating directly with the host OS.
- Consistent shell across all platforms Emacs is running on
- Ability to redirect the output of commands directly to a buffer
- Does not support interactive programs like =top=, but creates an =ansi-term=
  instance directly from eshell.
- every command you type is first filtered through eshell's own emulation layer,
  then through emacs own interactive commands and then finally through programs
  in your =$PATH= or in the current directory. So for example typing =dired .=
  will open a =M-x dired= session in the current directory.

*** =M-x ansi-term= - terminal emulator

- terminal emulator so you can run interactive programs like `top` or even `vim`
  or `emacs`
- Best terminal experience in Emacs, but you'll have to switch modes to use it
  as a buffer
- Quite slow and not supporting all shell features

| Keybinding | Description                                                |
|------------+------------------------------------------------------------|
| =C-c C-j=  | Switch to line mode (like typical emacs buffer)            |
| =C-c C-k=  | Switch to character mode (like a normal terminal emulator) |
|------------+------------------------------------------------------------|
