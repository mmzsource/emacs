...
easily switching back to previous window configurations. (See config.org file).

Migt also be nice to activate the *windmove* package to switch windows by
pressing =S-<left>=, =S-<right>=, =S-<up>= & =S-<down>=.
